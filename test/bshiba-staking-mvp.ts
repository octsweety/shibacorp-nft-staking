...
import { BShibaStakingMVP } from '../types/ethers-contracts/BShibaStakingMVP';
import { BShibaStakingMVP__factory } from '../types/ethers-contracts/factories/BShibaStakingMVP__factory';
...
describe('Testing ShibaCorp NFT ...', () => {
    ...
    let bshibaPool: BShibaStakingMVP;
    ...
    it('Claim', async() => {
        ...
        await bshibaPool.connect(deployer).claim();
        await bshibaPool.connect(account1).claim();
        await bshibaPool.connect(account2).claim();

        if (Number(count1) > 0) {
            const reward1 = await nftInst1.tokens(deployer.address);
            console.log("reward1: ", reward1);
        }

        if (Number(count2) > 0) {
            const reward2 = await nftInst1.tokens(account1.address);
            console.log("reward1: ", reward2);
        }

        if (Number(count3) > 0) {
            const reward3 = await nftInst1.tokens(account2.address);
            console.log("reward1: ", reward3);
        }
    });
    ...
    it('WithdrawAll', async() => {
        ...
        expect(await bshiba.balanceOf(deployer.address)).eq(beforeBalance.add(parseShiba('100000000')));
